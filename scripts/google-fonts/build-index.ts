...
if (args.curated !== undefined) {
  ...
  console.log(`Wrote ${families.length} downloadable manifests, index, script shards, and NOTICE.`)
  ...
  await Bun.write(`${out}/CATALOG-REFRESH.md`, [
    ...
    '',
    'Downloadable families are OFL-1.1 / Apache-2.0 manifests under `families/` with pinned gstatic WOFF2 URLs, generated by `bun scripts/google-fonts/generate-family-manifests.ts` then `bun scripts/google-fonts/build-index.ts --curated`. UFL and CJK remain browse-only.',
    '',
    'Refresh manually with `bun scripts/google-fonts/build-index.ts --github`, inspect the metadata/provenance diff, regenerate manifests, then run `--curated`.',
    '',
